...
/**
 * Summary screen to display the player's performance on the WW2 play level
 */
export class PlayWW2SummaryView implements View {
	private group: Konva.Group;
    private model: PlayWW2Model;
    private rankStars: Konva.Text;

	constructor(onHomeClick: () => void, onPlayClick: () => void, ww2Model: PlayWW2Model) {
		...
		const subtext = new Konva.Text({
			x: STAGE_WIDTH / 2,
			y: STAGE_HEIGHT / 3 + 50,
			text: "Well done! You've completed the WWII level",
			fill: "black",
			fontStyle: "bold",
			stroke: "white",
			fontSize: 28,
